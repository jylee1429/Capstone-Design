...
void LoadCell_Setting(void){
  ...
  Set_Scale(&loadcell_right, scale_factor);
  Set_Scale(&loadcell_left, scale_factor);
}

int32_t Get_Force_Right(void){
  return Get_Units(&loadcell_right, 1) * 0.453592;      // [kg]
  // return LPF_Force(&loadcell, input_force, THETA);
}

int32_t Get_Force_Left(void){
  return Get_Units(&loadcell_left, 1) * 0.453592;      // [kg]
  // return LPF_Force(&loadcell, input_force, THETA);
}