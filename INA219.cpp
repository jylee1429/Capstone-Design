...
{
  if ((_address < 0x40) || (_address > 0x4F))                // address : 0x40 ~ 0x43
    return false;
  _wire->beginTransmission(_address);                        // send to check signal to _address
  return ( _wire->endTransmission() == 0);                   // check return signal, if return signal == 0, return true
}
