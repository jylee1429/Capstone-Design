...
void Current_Sensor_Init(void){
  int i;
  Init(&dev_arr[0], 0x40, &Wire);
  Init(&dev_arr[1], 0x41, &Wire);
  Init(&dev_arr[2], 0x43, &Wire);
  Init(&dev_arr[3], 0x44, &Wire);
...
void Get_Current_right(float* right_u, float* right_v, float* right_w){
  *right_u = GetCurrent(&dev_arr[0]);
  *right_v = GetCurrent(&dev_arr[1]);
  *right_w = - (*right_u) - (*right_v);
}

void Get_Current_left(float* left_u, float* left_v, float* left_w){
  *left_u = GetCurrent(&dev_arr[2]);
  *left_v = GetCurrent(&dev_arr[3]);
  *left_w = - (*left_u) - (*left_v);
}