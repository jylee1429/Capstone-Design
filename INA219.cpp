...
INA219 ina219_dev[DEVICE_NUM];
uint8_t address[DEVICE_NUM] = { 0x40, 0x41, 0x43, 0x44 };
...
void Current_Sensor_Setting(void){
  int i;
  for(i = 0; i < DEVICE_NUM; i++){
    Init(&ina219_dev[i], address[i], &Wire);
  }

  // connect checking
  for(i = 0; i < DEVICE_NUM; i++){
    if(!Begin(&ina219_dev[i])){
...
float Calculate_Current_Right(void){                                    
  // float right_u, right_v, right_w;
  float right_current[3];
  float*  p_arr = &right_current[0];
  float right_cur;

  right_current[0] = GetCurrent(&ina219_dev[0]);                             // right_current_U
  right_current[1] = GetCurrent(&ina219_dev[1]);                             // right_current_V
  right_current[2] = - (right_current[0]) - (right_current[1]);               // right_current_W (U + V + W = 0) 
  // calculate right current
  right_cur = Get_Maximum(p_arr);

  return right_cur;
}

float Calculate_Current_Left(void){                                               
  // float left_u, left_v, left_w;
  float left_current[3];
  float*  p_arr = &left_current[0];
  float left_cur;
  left_current[0] = GetCurrent(&ina219_dev[2]);                               // left_curent_U
  left_current[1] = GetCurrent(&ina219_dev[3]);                               // left_curent_V
  left_current[2] = - (left_current[0]) - (left_current[1]);                  // left_curent_W  (U + V + W = 0) 
  // calculate left current

  left_cur = Get_Maximum(p_arr);
  return left_cur;
}

// void Get_Current_Right(void){
//   current_right = Calculate_Current_Right();
// }

// void Get_Current_Left(void){
